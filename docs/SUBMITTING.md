...
- **package** is not something you write — leave it out. The tooling sniffs the file's own bytes during validation and ingest (a zip starts with `PK`, anything else is a dll) and writes `package` into the compiled manifest for you, so it can never disagree with the actual file.
