...
- **parity_required** is present and a boolean. It's mandatory, so you answer it deliberately rather than inheriting a default: `true` means a client joining a server that runs your mod must have this exact version or the join is refused. Set it `false` when your mod's client and server halves work independently, so a player without it costs nobody else anything, and the launcher will offer it rather than insist on it. `false` is only meaningful when both `client_side` and `server_side` are true, and setting it `false` when the two halves actually have to agree produces broken sessions rather than a clear rejection, so only reach for it when you're sure. If you're not, `true` is the safe answer.
